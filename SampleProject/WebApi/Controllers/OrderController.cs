...
namespace WebApi.Controllers
{
    ...
    public class OrderController : BaseApiController
    {
        ...
        {
            var order = _createOrderService.Create(orderId, model.Name, model.Email);
        ...
        public HttpResponseMessage UpdateOrder(Guid orderId, [FromBody] OrderModel model)
        {
            ...
            _updateOrderService.Update(order, model.Name, model.Email);
            return Found(new OrderData(order));
        }
        ...
        [Route("list")]
        [HttpGet]
        public HttpResponseMessage GetOrders(int skip, int take, string name = null, string email = null)
        {
            var orders = _getOrderService.GetOrders(name, email)
                                       .Skip(skip).Take(take)
        ...
        [Route("list/tag")]
        [HttpGet]
        public HttpResponseMessage GetOrdersByTag(string tag)
        {
            var orders = _getOrderService.GetOrdersByTag(tag)
                                  //.Skip(skip).Take(take)
                                  //.Select(q => new OrderData(q))
                                  .ToList();
            return Found(orders);
